...
* Live migrate a VM between two storage classes
...
. We can now see that the virtual machine is living on a new node, in the image _worker-cluster-5dktk-3_, and that the pod itself now resides in a new pod with a new unique ID as the live migration process transfers the VM instance to a new pod on a new worker node nondisruptively.
+
