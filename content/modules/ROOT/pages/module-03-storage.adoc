...
Some physical storage system examples with supported CSI drivers include: Dell/EMC, Fujitsu, Hitachi, NetApp, and Everpure Storage. 

Some software-defined storage examples with supported CSI drivers include: IBM Fusion Data Foundation, OpenShift Data Foundation (ODF), and Portworx. 

Some software-defined datastore-based solutions include: Arctera Infoscale and IBM Fusion Access for SAN. 
...
This workshop segment will explore Persistent Volume Claims (PVCs), which are used to request storage from the provider and store a VM disk when using the Kubernetes CSI paradigm. Many storage providers also support advanced storage features like volume snapshots and clones with their drivers. Be sure to check with your preferred storage vendor to verify the features supported by their CSI driver and storage device.
...
In this lab, we are going to take a closer look at the storage behind the virtual machine we just created earlier: *fedora01*.
...
.. The PVC is currently bound successfully
...
. Navigate back to *Virtualization -> VirtualMachines* in the left-side menu. Expand the project *vmexamples-{user}* in the center column and highlight the *fedora01* virtual machine.
+
...
+
NOTE: There is a warning about the *cloudinitdisk* not being included in the snapshot. This is expected and happens because it is an ephemeral disk only used for the initial boot of the VM.

. A name will be auto-generated for the Snapshot. Press *Save* and wait until the *status* shows as *Succeeded*.
+
...
+
NOTE: There are copy icons next both the *User name* and *Password* and a *Paste to console* button available all available here, which makes the login process much easier. You may be prompted to allow this action to take place, click the *Allow* button if prompted.
...
. Using the *Actions -> Control* dropdown menu or the shortcut button in the top right corner, *Stop* the VM. This process can take a long time since it attempts a graceful shutdown and the machine is in an unstable state. If you click on the *Actions -> Control* dropdown menu again you will have the option to *Force stop*. Please make use of this option in order to continue with the lab.
...
. Return to *Overview* tab, and start the VM using either the *Start* button or the *Actions -> Control -> Start* dropdown.
+
